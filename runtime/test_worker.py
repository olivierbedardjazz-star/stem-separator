...
mode = 'karaoke' if '--karaoke' in sys.argv else 'stems'
...
with tempfile.TemporaryDirectory(prefix='proof-', dir='build/StemRuntime') as d:
    ...
    ready = json.loads(line); assert ready['type'] == 'ready' and ready.get('device') == 'mps', ready
    request = dict(protocolVersion=1,type='separate',jobID=job,
                   ...
                   outputDirectory=str(root/'worker-output'),modelID='htdemucs',device='mps',outputMode=mode)
    ...
    if status == 21 and '--allow-unavailable' in sys.argv:
        assert events[-1].get('code') == 'mpsUnavailable' and not (root/'worker-output').exists()
        print('NOT TESTED: MPS inference unavailable on this host; unavailable-GPU failure path passed.', flush=True)
        sys.exit(0)
    assert status == 0
    progress=[e for e in events if e['type']=='progress']
    assert progress[-1]['completedUnits'] == progress[-1]['totalUnits'] == 3
    result=events[-1]; assert result['type']=='result' and result.get('device')=='mps' and result.get('outputMode')==mode
    names = ['karaoke'] if mode == 'karaoke' else ['vocals', 'drums', 'bass', 'other']
    assert sorted(s['name'] for s in result['stems']) == sorted(names)
    assert sorted(x.name for x in (root/'worker-output').iterdir()) == sorted(n+'.f32le' for n in names)
    ...
    print('PASS: frozen MPS worker, empty HOME, minimal PATH, network denied, only the requested finite stereo outputs (' + mode + ').', flush=True)
