# Stem Separator Third-Party Notices

Bundled software and model attribution.

Python 3.11 is distributed under the PSF license and included third-party notices. PyInstaller uses GPL-2.0 with its bootloader exception. Full collected license texts are bundled in RuntimeLicenses.

Demucs 4.0.1 code is MIT licensed. Model htdemucs checkpoint 955717e8-8726e21a.th comes from https://dl.fbaipublicfiles.com/demucs/hybrid_transformer/955717e8-8726e21a.th . SHA-256: 8726e21a993978c7ba086d3872e7608d7d5bfca646ca4aca459ffda844faa8b4.

HTDemucs checkpoint 955717e8 is distributed under the historical MIT declaration at https://huggingface.co/adefossez/HTDemucs/blob/bf35a81b663819a8255c8fefee17f9d812b786b5/README.md . Copyright (c) Meta Platforms, Inc. and affiliates. The complete MIT notice is included as HTDemucs-MIT.txt.

Corresponding sources and library modification information are provided in RUNTIME_SOURCE_AND_RELINKING.md and the source archive alongside each release.

Build dependencies are included in this inventory as well as runtime dependencies. Packaging does not imply that every listed package is executed. Native libraries may have additional notices in the complete texts. This inventory does not constitute completed public-release license clearance.

altgraph 0.17.5 — MIT
antlr4-python3-runtime 4.9.3 — BSD
cloudpickle 3.1.2 — BSD-3-Clause
demucs 4.0.1 — MIT License
dora_search 0.1.12 — MIT
einops 0.8.2 — MIT
filelock 3.32.5 — MIT
fsspec 2026.7.0 — BSD-3-Clause
Jinja2 3.1.6 — Not declared
julius 0.2.8 — MIT License
lameenc 1.8.4 — LGPL-3.0-or-later
macholib 1.16.4 — MIT
MarkupSafe 3.0.3 — BSD-3-Clause
mpmath 1.3.0 — BSD
networkx 3.6.1 — BSD-3-Clause
numpy 1.26.4 — Copyright (c) 2005-2023, NumPy Developers.
omegaconf 2.3.1 — Not declared
openunmix 1.3.0 — MIT
packaging 26.3 — Apache-2.0 OR BSD-2-Clause
pip 24.0 — MIT
pyinstaller 6.16.0 — GPLv2-or-later with a special exception which allows to use PyInstaller to build and distribute non-free programs (including commercial ones)
pyinstaller-hooks-contrib 2026.7 — Not declared
PyYAML 6.0.3 — MIT
retrying 1.4.2 — Apache-2.0
setuptools 82.0.0 — MIT
submitit 1.5.4 — Not declared
sympy 1.13.1 — BSD
torch 2.5.1 — BSD-3-Clause
torchaudio 2.5.1 — Not declared
tqdm 4.70.0 — MPL-2.0 AND MIT
treetable 0.2.6 — Unlicense license
typing_extensions 4.16.0 — PSF-2.0
wheel 0.46.3 — MIT

Sparkle: pinned revision 79bc9e872948e47877e76f194cb0c8e0412b0b90, MIT. See included Sparkle-LICENSE.txt.

To inspect full notices, use Show Package Contents on Stem Separator.app, then Contents/Resources/RuntimeLicenses.
